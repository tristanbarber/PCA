...
int main(int argc, char *argv[]) {
    ...
    int base_rows = (HEIGHT - 2) / nprocs;
    int rem = (HEIGHT - 2) % nprocs;
    ...
    for (int i = 0; i < nprocs; i++) {
        ...
        rdispls[i] = rdispls_tracker + WIDTH;
    ...
    if (rank == 0) {
        input = malloc(HEIGHT * WIDTH * sizeof(uint8_t));
        output = calloc(HEIGHT * WIDTH, sizeof(uint8_t));
        read_image(argv[1], input);
    }

    uint8_t *local_in = calloc(local_rows * WIDTH, sizeof(uint8_t));
    uint8_t *local_out = calloc(local_rows * WIDTH, sizeof(uint8_t));

    begin = now();

    MPI_Scatterv(input, sendcounts, displs, MPI_UINT8_T,
                 local_in, sendcounts[rank], MPI_UINT8_T,
                 0, MPI_COMM_WORLD);

    sobel(local_in, local_out, local_rows);

    MPI_Gatherv(local_out + WIDTH, recvcounts[rank], MPI_UINT8_T,
                output, recvcounts, rdispls, MPI_UINT8_T,
                0, MPI_COMM_WORLD);

    if (rank == 0) {
        write_image("output_parallel.txt", output);
        end = now();
